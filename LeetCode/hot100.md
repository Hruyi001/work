...
public:
    ...
    bool dfs(vector<vector<char>>& board, int i, int j, int u, string word) { 
        ...
        for(int k = 0; k < 4; k++) {
            ...
            if(x < 0 || x >= n || y < 0 || y >= m ||   board[x][y] != word[u + 1]) continue;
...
// 剪枝策略2:如果剩余字符数字已经小于还需要删除的字符个数，剪枝。（因为有非括号字符，所以还可以标记剩余的左括号和右括号个数是否小于当前要删除的个数，得到更早的剪枝） 
    ...
    void dfs(string s, int u, int l, int r) {
        ...
        for(int i = u; i < n; i++) {
            // 忽略非括号字符
            if(s[i] != '(' && s[i] != ')') continue;
            // 1.如果是第一个，或者2.如果不相等就开始，（相等的话就跳过，已经由2在递归中完成了）
            if(i == u || s[i] != s[i - 1]) {
                string cur = s;
                // 删除的是cur字符串
                cur.erase(i, 1);
